...
import p4vasp.piddle.pdfutils as pdfutils
import p4vasp.piddle.pdfdoc as pdfdoc
import p4vasp.piddle.pdfmetrics as pdfmetrics
import p4vasp.piddle.pdfgeom as pdfgeom
        ...
        while not done:
            ...
            if x[0] == 0xFF:            #found marker
                ...
                if x[0] in validMarkers:
                    ...
                    if x[0] != 8:
                        raise RuntimeError(' JPEG must have 8 bits per component')
                    ...
                    raise RuntimeError(' Unsupported JPEG marker: %0.2x' % x[0])
...
"""
        ...
        if direction in [0,90,180,270]:
            ...
            raise RuntimeError(' directions allowed are 0,90,180,270')

        if dimension in ['H', 'V']:
            dimension_arg = '/Dm /%s' % dimension
        else:
            raise RuntimeError('dimension values allowed are H and V')

        if motion in ['I','O']:
            motion_arg = '/M /%s' % motion
        else:
            raise RuntimeError('motion values allowed are I and O')
        ...
        try:
            ...
            raise RuntimeError('Unknown Effect Name "%s"' % effectname)
            self._pageTransitionString = ''
            return
