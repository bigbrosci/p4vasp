...
_FLOAT_PROPS = {"angle", "fraction", "pulse_step", "value", "lower", "upper", "step_increment", "page_increment", "xalign", "yalign"}
...
class XML:
    ...
    def _build_widget(self, elem, parent=None, internal_child=None):
        ...
        if isinstance(widget, gtk.OptionMenu):
            widget.set_history(int(props.get("history", "0")))
        ...
        for name, raw in props.items():
            ...
            try:
                if name == "title" and hasattr(widget, "set_title"):
                    ...
                    widget.set_xalign(float(props.get("xalign", 0.5)))
                    widget.set_yalign(float(props.get("yalign", 0.5)))
                    ...
                    widget.set_size_request(value, widget.get_size_request().height)
                elif name == "height_request":
                    widget.set_size_request(widget.get_size_request().width, value)
                elif name == "events":
                    widget.add_events(value)
                elif name == "tooltip":
                    widget.set_tooltip_text(value)
                elif name == "orientation" and isinstance(widget, Gtk.ProgressBar):
                    widget.set_orientation(Gtk.Orientation.VERTICAL if raw in (
                        "GTK_PROGRESS_BOTTOM_TO_TOP", "GTK_PROGRESS_TOP_TO_BOTTOM") else Gtk.Orientation.HORIZONTAL)
                    widget.set_inverted(raw in ("GTK_PROGRESS_RIGHT_TO_LEFT", "GTK_PROGRESS_TOP_TO_BOTTOM"))
                    ...
                    img.show()
                elif name == "adjustment" and isinstance(widget, (Gtk.Range, Gtk.SpinButton)):
                    widget.set_adjustment(Gtk.Adjustment(*map(float, raw.split())))
                ...
                self.property_errors.append((widget.get_name(), name, str(error)))
    ...
    def _add_child(self, parent, child, child_elem):
        ...
        packing_element = child_elem.find("packing")
        packing = _properties(packing_element if packing_element is not None else ET.Element("packing"))
        if isinstance(parent, gtk.FileSelection):
            return
        ...
        if isinstance(parent, Gtk.Frame) and (
            ...
            or packing.get("type") == "label_item"
        ):
        ...
        if isinstance(parent, Gtk.Notebook):
            if packing.get("type") == "tab" or child_elem.attrib.get("type") == "tab":
                page = parent.get_nth_page(parent.get_n_pages() - 1)
                if page is not None:
                    parent.set_tab_label(page, child)
                return
