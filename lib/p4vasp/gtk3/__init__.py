"""GTK3 runtime and adapters for p4vasp UI resources and table models."""
...
from gi.repository import Gdk as _Gdk, GdkPixbuf as _GdkPixbuf
from gi.repository import GLib as _GLib, GObject as _GObject, Gtk as _Gtk
...
class GenericTreeModel:
    ...
    def get_iter(self, path):
        if isinstance(path, str):
            path = tuple(int(part) for part in path.split(":"))
        elif isinstance(path, _Gtk.TreePath):
    ...
    def get_value(self, node, column):
        return self.on_get_value(node, column)

    def _refresh_stores(self):
        for store in self._stores:
            store.clear()
            _populate_tree_model(self, store)
    ...
    def foreach(self, callback, user_data=None):
        def visit(parent):
            for node, row in _generic_model_rows(self, parent):
                if callback(self, self.on_get_path(node), node, user_data):
                    return True
                if self.on_iter_has_child(node) and visit(node):
                    return True
            return False
        visit(None)
...
def _generic_to_tree_model(model):
    ...
    _populate_tree_model(model, store)
    model._stores.append(store)
    return store


def _populate_tree_model(model, store):
    if isinstance(store, _Gtk.ListStore):
...
# PyGObject reserves these obsolete module names with error sentinels. Remove
# only those sentinels so our forwarding modules remain usable by old plugins.
for _legacy_name in ("gtk", "gtk.gdk", "gobject", "pango"):
    if type(sys.modules.get(_legacy_name)).__name__ == "_DummyStaticModule":
